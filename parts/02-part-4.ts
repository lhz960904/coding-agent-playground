...
async function runAgent(provider: LLMProvider, providerLabel: string, userMessage: string, log: (s: string) => void, signal?: AbortSignal) {
  log(`\x1b[36m[provider=${providerLabel}]\x1b[0m ${userMessage}`);
  ...
  while (true) {
    ...
    log(`\x1b[33m[step]\x1b[0m provider.invoke ...`);
    ...
    if (toolUses.length === 0) {
      ...
      log(`\n\x1b[32m[assistant @ ${providerLabel}]\x1b[0m ${text}\n`);
      return;
    }
    log(`\x1b[35m[tool_use]\x1b[0m ${toolUses.map((u) => `${u.name}(${JSON.stringify(u.input)})`).join(", ")}`);
